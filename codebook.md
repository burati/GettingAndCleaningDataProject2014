...
Saving to TXT file `DatasetHumanActivityRecognitionUsingSmartphones.txt`.
------------


```r
txtfile <- file.path(folder, "DatasetHumanActivityRecognitionUsingSmartphones.txt")
write.table(tidydata, txtfile, quote=FALSE, sep="\t", row.names=FALSE)
```
